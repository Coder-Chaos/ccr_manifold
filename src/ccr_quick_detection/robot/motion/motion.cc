...
namespace ccr_quick_detection
{
  ...
  void Motion::MoveDownLeak()
  {
    ...
    {
      tmotors_->at(i).param_->dir = 1;
      tmotors_->at(i).param_->tmtSetVel = GearBox() * 0.6;
      tmotors_->at(i).param_->tmtSetKD = Kd;
      tmotors_->at(i).SetVel(tmotors_->at(i).param_->dir, tmotors_->at(i).param_->tmtSetVel,
                                 tmotors_->at(i).param_->tmtSetKD);
    }
  ...
  void Motion::Stop()
  {
    bool stop_flag = false;
    // check Vel_min=0.0125
    //if (fabs(odometer_ - odometer_0) > 0.001f)
    //{
    for (auto tmotor : *tmotors_)
    {
    ...
    if (stop_flag == false)
    {
      ...
      /*for (int i = 0; i < 2; i++)
      {
        tmotors_->at(i).param_->tmtSetVel = 0;
        tmotors_->at(i).param_->tmtSetKD = Kd;
        tmotors_->at(i).param_->tmtSetTor = -1.8f;
        tmotors_->at(i).SetVelToq(tmotors_->at(i).param_->tmtSetVel, tmotors_->at(i).param_->tmtSetKD, 
                                   tmotors_->at(i).param_->tmtSetTor);
      }
      for (int i = 2; i < 4; i++)
      {
        tmotors_->at(i).param_->tmtSetVel = 0;
      ...
      {
        tmotor.param_->dir = 0;
        tmotor.param_->tmtSetPos = tmotor.param_->tmtGetPos;
        tmotor.param_->tmtSetKP = Kp;
        tmotor.param_->tmtSetKD = Kd;
        tmotor.SetPos(tmotor.param_->tmtSetPos, tmotor.param_->tmtSetKP,
                      tmotor.param_->tmtSetKD);
        tmotor.ExitMotorMode();
        //usleep(3*1000);
        tmotor.EnterMotorMode();
      ...
      for (int i = 0; i < 2; i++)
      {
        tmotors_->at(i).param_->tmtSetVel = -0.15;
        tmotors_->at(i).param_->tmtSetKD = Kd;
        tmotors_->at(i).SetVel(tmotors_->at(i).param_->tmtSetVel, tmotors_->at(i).param_->tmtSetKD);
      }
      for (int i = 2; i < 4; i++)
      {
        tmotors_->at(i).param_->tmtSetVel = 0.15;
