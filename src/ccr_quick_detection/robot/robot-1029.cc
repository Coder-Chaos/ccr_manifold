...
namespace ccr_quick_detection
{
  ...
  void Robot::TFminiRecv()
  {
    ...
    Range_.frame_id = id;
    //printf("\rmin=%4.2f; max=%4.2f.\n", Range_.min_range, Range_.max_range);*/
     
    Range_->radiation_type = INFRARED;
    Range_->field_of_view = 0.04f;
    Range_->min_range = 0.3f;
    Range_->max_range = 12.0f;
    //Range_->frame_id.assign(id);
    Range_->frame_id = id;
    //memcpy(&Range_->frame_id, &id, sizeof(Range_->frame_id));
    //strcpy(&Range_->frame_id, &id);
    ...
    tfmini_thread_ = std::thread([&]()
                                 {
                                   ...
                                   while (stop_tfmini_signal_ == false)
                                   {
                                     ...
                                     {
                                       log_error("Data validation error!");
                                     }
                                     usleep(5000);
                                   }
